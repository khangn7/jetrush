...
export class Shape {
    ...
    rotate_around_vector(rotate_angle, theta, phi) {
        ...
        const point_count = model.points.length;
        for (let i = 0; i < point_count; i++) {
            let new_point = rotate_v_around_k(model.points[i], rotate_angle, theta, phi);
            new_points.push(new_point);
        }
     ...
     */
     draw_surface(surface_index, color, orthographic=!PERSPECTIVE, draw_normal=false) {
        ...
        // sunlight(color, surface_ref.normal)
        let light_index = 0.5 + (-dotProduct(LIGHT_V, surface_ref.normal) + 1)/4;
        color = color.map(x => x * light_index)

        // fade
        let center = surface_ref.center;
        let dist = Math.sqrt(center.x**2 + center.y**2)
        // if (dist > 0.5 * FADE_MAX) {
        //     color = color.map(x => x + (256 - x) * (dist / FADE_MAX))
        // }
        color = color.map(x => x + (256 - x) * (dist / FADE_MAX))
        console.log(surface_ref.corner1)
...
export class CubeShape extends Shape {
    ...
    draw_surfaces(color=false) {
        const ctx = this.canvas.getContext("2d");
        color = color ? color : [143, 188, 143]; // default
        for (let i = 0; i < 6; i++) {
