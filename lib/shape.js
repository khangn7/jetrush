...
export class Shape {
     ...
     */
     draw_surface(surface_index, color, orthographic=!PERSPECTIVE, draw_normal=false) {
        ...
        let light_index = (-dotProduct(LIGHT_V, surface_ref.normal) + 1)/2;
        color = color.map((x) => String(x * light_index))
...
export class CubeShape extends Shape {
    ...
    draw_surfaces(color=false) {
        // let colors = ["crimson", "darkseagreen", "darkcyan", "darkolivegreen", "chocolate", "blueviolet"];
        const ctx = this.canvas.getContext("2d");
        color = color ? color : [143, 188, 143];
        for (let i = 0; i < 6; i++) {
