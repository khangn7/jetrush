...
export class Shape {
    ...
    constructor(canvas, coord_object, coord_class) {
        ...
        this.model_k_hat = new Vector(0, 0, 1);
        this.display_k_hat = new Vector(0, 0, 1); // used to track orientation of display model
        this.display_k_hat_theta = Math.PI * 0.5;// angle made with positive y axis
        this.display_k_hat_phi = 0; // angle made with positive z axis
    ...
    rotate(rotate_angle, rotation_axis, use_current_coords=false) {
        ...
        // Math.atan2(y,x) gives angle between ray from origin to (x,y) and positive x axis
        // use atan2 specifically because it outputs in range [-pi, pi]
        // note: Math.atan2(y,x) for some reason not Math.atan2(x,y)
        let new_k_hat = linear_combination3d(this.model_k_hat, x_col, y_col, z_col);
        this.display_k_hat = new_k_hat;
        let h = Math.sqrt(new_k_hat.x**2 + new_k_hat.z**2)
        this.display_k_hat_theta = Math.atan2(h, new_k_hat.y) 
        this.display_k_hat_phi = Math.atan2(new_k_hat.x, new_k_hat.z)
