...
export class blockOfBuildings {
    ...
    */
    constructor(canvas_elem, info) {
        // contains all rows of buildings with world_pos.x >= 0
    ...
    perform_on_all(func) {
        // do on left side first for drawing
    ...
    move(x_change, y_change, z_change) {
        ...
        let rb = this.right_buildings,
            lb = this.left_buildings;
        // let half_b_w = this.building_w;
        for (let row_i in rb) {
            // if first building in row i is...
            if (rb[row_i][0].world_pos.x < 0) {
                console.log("r", row_i)
                lb.push(rb.shift());
            }
        }
