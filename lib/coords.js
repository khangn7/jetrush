...
//      * @param {Number} theta normal vector theta
//      * @param {Number} phi normal vector phi
//      */
//     constructor(corner1, corner2, corner3, theta, phi) {
...
export class Cube_coords { // could be used for other things than cubes
    ...
    constructor(_points, theta, phi) {
        ...
        let angles = translate_angles(theta, phi),
            i_angles = angles[0],
            j_angles = angles[1];
        ...
        this.surfaces = [
            // triangle [point 1, point 2, point 3, normal vector theta, normal vector phi]

            // back face. start on bottom left go anti-clockwise
            [ _points[1], _points[3], _points[0], map_angle_to_range(theta + Math.PI), map_angle_to_range(phi + Math.PI)],
            [ _points[1], _points[3], _points[2], map_angle_to_range(theta + Math.PI), map_angle_to_range(phi + Math.PI)],

            // front face
            [ _points[4], _points[6], _points[7], map_angle_to_range(theta), map_angle_to_range(phi)],
            [ _points[4], _points[6], _points[5], map_angle_to_range(theta), map_angle_to_range(phi)],

            // right face
            [ _points[1], _points[6], _points[5], map_angle_to_range(i_angles[0]),map_angle_to_range(i_angles[1])],
            [ _points[1], _points[6], _points[2], map_angle_to_range(i_angles[0]), map_angle_to_range(i_angles[1])],

            // left face
            [ _points[4], _points[3], _points[7], map_angle_to_range(i_angles[0] + Math.PI), map_angle_to_range(i_angles[1] + Math.PI)],
            [ _points[4], _points[3], _points[0], map_angle_to_range(i_angles[0] + Math.PI), map_angle_to_range(i_angles[1] + Math.PI)],

            // top face
            [ _points[6], _points[3], _points[7], map_angle_to_range(j_angles[0]), map_angle_to_range(j_angles[1])],
            [ _points[6], _points[3], _points[2], map_angle_to_range(j_angles[0]), map_angle_to_range(j_angles[1])],

            // bottom face
            [ _points[5], _points[0], _points[1], map_angle_to_range(j_angles[0] + Math.PI), map_angle_to_range(j_angles[1] + Math.PI)],
            [ _points[5], _points[0], _points[4], map_angle_to_range(j_angles[0] + Math.PI), map_angle_to_range(j_angles[1] + Math.PI)]
